...
module.exports.run = async (client, msg, args, config) => {
    if(cooldown.has(msg.author.id)) {
        ...
        fs.readFile('./accounts/mcdo.txt', 'utf8', function(err, data) {
            if (err) throw err;
            ...
            fs.writeFile('./accounts/mcdo.txt', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });

            let embed = new Discord.RichEmbed()
            .setTitle('Voici votre nouveau compte McDo !')
            ...
            msg.reply(':white_check_mark: Votre compte McDo vient d\'etre généré en MP!')
                .then(m => {
                    setTimeout(() => {
                        m.delete();
                    }, 900000);
